(require-extension test readline srfi-1 dummy-user)
(test-begin "Readline API")
...
|#
(test-end)
(test-begin "Legacy API")
(test "(use-legacy-bindings) == (void)"
      (void)
      (use-legacy-bindings))

(define tester-inigo (make-dummy-user '("Hello, my name is Inigo Montoya." "You killed my father." "Prepare to die.")))
(test-end)
(test-exit)
